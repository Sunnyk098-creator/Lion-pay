...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!key || !paytm || !amount) {
            return res.status(400).json({ status: "error", message: "Missing parameters! key, paytm or amount required." });
        }
        ...
        if (!receiverSnap.exists()) return res.status(404).json({ status: "error", message: "Receiver mobile number is not registered!" });
        ...
        updates[`transactions/${txnId}`] = { 
            ...
            status: "Success", date: exactDate, timestamp: Date.now(), icon: "fa-code", 
            color: "gray", name: receiverData.name || paytm, number: paytm,
            senderName: adminData.name || adminPhone, senderId: adminPhone, receiverId: paytm, isApi: true
        };

        await update(ref(db), updates);

        let rName = receiverData.name || paytm; let aName = adminData.name || adminPhone;
        if (adminData.tgUserId) sendTelegramMsg(adminData.tgUserId, `🤖 API Payment Sent!\nTo: ${rName}\nAmount: ₹${withdrawAmount}\nTxn ID: ${txnId}`);
        if (receiverData.tgUserId) sendTelegramMsg(receiverData.tgUserId, `💰 API Payment Received!\nFrom: ${aName}\nAmount: ₹${withdrawAmount}\nTxn ID: ${txnId}`);

        return res.status(200).json({ status: "success", message: `Payment successful to ${paytm}`, data: { transaction_id: txnId, amount: withdrawAmount, receiver: paytm, sender: adminPhone } });
