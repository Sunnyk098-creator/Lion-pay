...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'EXECUTE_TXN') {
            ...
            else if (data.mode === 'CARD_LOAD') { updates[`users/${data.sender}/balance`] = increment(-Number(data.amount)); updates[`users/${data.sender}/cardBalance`] = increment(Number(data.amount)); } 
            else if (data.mode === 'CARD_WITHDRAW') { updates[`users/${data.sender}/cardBalance`] = increment(-Number(data.amount)); updates[`users/${data.sender}/balance`] = increment(Number(data.amount)); } 
            else if (data.mode === 'CARD_PAY') { updates[`users/${data.sender}/cardBalance`] = increment(-Number(data.amount)); } 
        ...
        if (action === 'GET_LIFAFA_INFO') {
            ...
            let lData = snap.val();
            // Check expiry logic dynamically for info
            if (Date.now() - lData.timestamp > 72 * 60 * 60 * 1000 && lData.status === 'ACTIVE') {
                lData.status = 'EXPIRED';
            }
            return res.json({ data: lData });
        }
        ...
        if (action === 'CLAIM_LIFAFA') {
            ...
            const result = await runTransaction(lifafaRef, (currentData) => {
                ...
                // Expiry System (72 Hours)
                if (Date.now() - currentData.timestamp > 72 * 60 * 60 * 1000) {
                ...
                // Verify Unique Code if Advanced Lifafa
                if (currentData.hasUniqueCode && currentData.uniqueCode !== data.uniqueCode) return;
            ...
            if (!result.committed) throw new Error("Lifafa invalid, expired, already claimed, or wrong code.");
            
            let lData = result.snapshot.val();
            if(lData.status === 'EXPIRED') throw new Error("This Lifafa has expired.");

            // Advanced Reward Logic
            if (lData.type === 'Standard') wonAmount = lData.amount;
            else if (lData.type === 'Scratch') wonAmount = Math.floor(Math.random() * (lData.maxAmount - lData.minAmount + 1)) + lData.minAmount;
            else if (lData.type === 'Toss') wonAmount = (data.tossSide === lData.winningSide) ? lData.tossWin : 0;
            else wonAmount = (lData.type === 'Random Amount') ? Math.floor(Math.random() * lData.amount) + 1 : lData.amount; // Old support
            ...
            await update(ref(db), updates); return res.json({ data: { reward: wonAmount, status: lData.status } });
        }

        // ============================ NEW ADVANCED LIFAFA CREATION ============================
        if (action === 'CREATE_ADVANCED_LIFAFA') {
            const uSnap = await get(ref(db, `users/${data.phone}`));
            if (!uSnap.exists() || uSnap.val().balance < data.totalDeduct) throw new Error("Insufficient Balance!");
            
            const newLifafa = { ...data.lifafaData, timestamp: Date.now(), status: 'ACTIVE', claimedUsers: 0 };
            const updates = { 
                [`users/${data.phone}/balance`]: increment(-data.totalDeduct), 
                [`lifafas/${data.lifafaData.id}`]: newLifafa, 
