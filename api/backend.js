...
export default async function handler(req, res) {
    ...
    try {
        ...
        // --- NEW PREMIUM LIFAFA SYSTEM BACKEND ---
        if (action === 'CREATE_LIFAFA') {
            let totalDeduct = 0;
            if(data.type === 'Scratch Card') {
            ...
            const updates = { 
                ...
                [`lifafas/${data.id}`]: newLifafa, 
                [`transactions/${data.txn.id}`]: data.txn,
                [`users/${data.phone}/my_lifafas/${data.id}`]: true
            };
            ...
            for(let id of ids) {
                ...
                if(s.exists()) {
                    ...
                    if(l.status === 'ACTIVE' && (Date.now() - l.timestamp > 72 * 60 * 60 * 1000)) {
                        l.status = 'EXPIRED';
                        await update(ref(db, `lifafas/${id}`), { status: 'EXPIRED' });
                    }
                    lifs.push(l);
                }
            }
            lifs.sort((a,b) => b.timestamp - a.timestamp);
            return res.json({ data: lifs });
        }
        ...
        if (action === 'GET_LIFAFA_INFO') {
            ...
            return res.json({ data: { 
                type: lData.type, channel: lData.channel, hasCode: (lData.code && lData.code !== ''),
                totalUsers: lData.totalUsers, claimedUsers: lData.claimedUsers, status: lData.status
            }});
        }
        ...
        if (action === 'CLAIM_LIFAFA') {
            let wonAmount = 0;
            let tossResultStr = '';
            const lifafaRef = ref(db, `lifafas/${data.id}`);
            ...
            const result = await runTransaction(lifafaRef, (currentData) => {
                ...
                if (currentData.type === 'Scratch Card') {
                    wonAmount = Math.floor(Math.random() * (currentData.maxAmount - currentData.minAmount + 1)) + currentData.minAmount;
                } else if (currentData.type === 'Coin Toss') {
                    let outcomes = ['Head', 'Tail'];
                    tossResultStr = outcomes[Math.floor(Math.random() * 2)];
                    if(tossResultStr === currentData.tossSide) wonAmount = currentData.amount;
                    else wonAmount = 0;
                } else {
                    wonAmount = currentData.amount;
                }
