...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'CREATE_LIFAFA') {
            ...
            // Auto generate ID if code is left blank
            let lifafaId = (data.code && data.code.trim() !== "") ? data.code.trim() : 'LIF' + Date.now().toString(36).toUpperCase() + Math.random().toString(36).substring(2, 6).toUpperCase();
        ...
        if (action === 'CLAIM_LIFAFA') {
            ...
            if (lData.type === 'Scratch') {
                let min = Number(lData.minAmount) || 1;
                let max = Number(lData.maxAmount) || Number(lData.amount) || 1;
                wonAmount = Math.floor(Math.random() * (max - min + 1)) + min;
            } else if (lData.type === 'Toss') {
                wonAmount = Math.random() < 0.5 ? Number(lData.amount) : 0;
            } else {
                wonAmount = Number(lData.amount);
            }
