...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'CREATE_LIFAFA') {
            ...
            const newLifafa = { id: data.code, creator: data.phone, type: data.type, amount: Number(data.amount), totalUsers: Number(data.totalUsers), claimedUsers: 0, timestamp: Date.now(), status: 'ACTIVE' };
        ...
        // --- LIFFAFA CLAIM FIX (runTransaction used) ---
        if (action === 'CLAIM_LIFAFA') {
            ...
            const updates = { [`users/${data.phone}/balance`]: increment(wonAmount), [`transactions/${data.txn.id}`]: { ...data.txn, amount: wonAmount } };
            await update(ref(db), updates);
            return res.json({ data: wonAmount });
        }

        // --- GIFT CLAIM FIX (runTransaction used) ---
        ...
        if (action === 'GAME_BET') {
            ...
            if(data.color === 'red') updates[`game_rounds/${data.roundId}/totalRed`] = increment(data.amount);
            else updates[`game_rounds/${data.roundId}/totalGreen`] = increment(data.amount);
            await update(ref(db), updates); return res.json({ data: "Success" });
        }
