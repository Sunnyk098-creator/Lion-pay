...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'EXECUTE_TXN') {
            ...
            // FIXED: Explicitly casting to Number to ensure mathematical addition in Firebase
        ...
        if (action === 'GAME_BET') {
            ...
            if(data.color === 'red') updates[`game_rounds/${data.roundId}/totalRed`] = increment(Number(data.amount)); else updates[`game_rounds/${data.roundId}/totalGreen`] = increment(Number(data.amount));
            if(data.txn) updates[`transactions/${data.txn.id}`] = data.txn;
            await update(ref(db), updates); return res.json({ data: "Success" });
        }
